...
The hardware specifications below reflect what's currently used for the full Radical Enterprise lab. The `../labs/` section of the repo is meant to break this down into smaller, standalone versions of what's implemented in the full lab, so it's possible to follow along with lower hardware requirements — each smaller lab is effectively a puzzle piece of the same technologies and configurations used here.
